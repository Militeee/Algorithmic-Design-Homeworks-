...
int main(int argc , char* argv[]){
    ...
    printf("Building heap time for dim=%d\t%lf\n", dim,get_execution_time(b_time, e_time) );
    //print_min_heap(&hp);

    clock_gettime(CLOCK_REALTIME, &b_time);
    remove_minimum(&hp);
    clock_gettime(CLOCK_REALTIME, &e_time);
    printf("Remove minimum time for dim=%d\t%lf\n", dim,get_execution_time(b_time, e_time) );
    
    clock_gettime(CLOCK_REALTIME, &b_time);
    min_heap_insert(&hp,250);
    clock_gettime(CLOCK_REALTIME, &e_time);
    printf("Heap insert time for dim=%d\t%lf\n", dim,get_execution_time(b_time, e_time) );
    //print_min_heap(&hp);
    
    clock_gettime(CLOCK_REALTIME, &b_time);
    decrease_key_min_heap(&hp,5,0);
    clock_gettime(CLOCK_REALTIME, &e_time);
    printf("Decrease key time for dim=%d\t%lf\n", dim,get_execution_time(b_time, e_time) );
    //print_min_heap(&hp);
