...
int
is_valid_node(max_heap* hp, size_t node)
{
  return hp->n >= node;
}

int
max_heap_minimum(max_heap* hp)
{
  return hp->array[get_root()];
}

void
swap_max_heap(max_heap* hp, size_t i, size_t m)
{
  int tmp = hp->array[i];
...
void
max_heapify(max_heap* hp, size_t i)
{
  size_t m = i, k = -1;
  while (k != m) {
    k = m;
...
void
decrease_key_max_heap(max_heap* hp, size_t i, int value)
{
  if (hp->array[i] >= value)
...
void
max_heap_insert(max_heap* hp, int value)
{
  hp->n++;
...
void
print_max_heap(max_heap* hp)
{
  print_array(hp->array, hp->n + 1);
}

max_heap
max_heap_build(int* array, size_t n)
{
  max_heap hp = { array, n };
  for (int i = n; i >= 0; i--)
    max_heapify(&hp, i);
