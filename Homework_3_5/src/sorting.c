...
bucketSort(float* array, int dim)
{
  ...
  // I've opted for insertionSort for the buckets, maybe for semplicity and
  // because I do not expect a large number of elemnts per bucket
  for (int i = 0; i < dim; i++)
...
int
select_pivot(int* array, int begin, int end)
{
...
select_i(int* array, int i, int begin, int end)
{
  ...
  // if the pivot is in the right place return it, otherwise search in the right
  // half
  if (i == k + 1)
    return array[k];
  if (i < k + 1)
