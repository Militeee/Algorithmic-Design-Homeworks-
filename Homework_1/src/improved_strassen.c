...
void impr_strassen_aux(float **C, const size_t C_f_row, const size_t C_f_col,
               ...
               float **G, const size_t G_f_row, const size_t G_f_col,
               const size_t n)
{
  if (n < (1<<6)) {
...
// S1 = B12 - B22
  sub_matrix_blocks(G, G1X, GX2, // S1
                    B,B1X,BX2,
                    B,B2X,BX2,
                    n2);

    // P1 = A11 x S1
  impr_strassen_aux(G, G1X, GX1, //P1
               A, A1X, AX1,
               G, G1X, GX2, //S1
               G,G2X,GX2,
               n2 );
  ...
  // S2 = A11 + A12
  sum_matrix_blocks(C, C1X, CX2, //S2
                    A,A1X,AX1,
                    A,A1X,AX2,
                    n2);
  
  // P2 = S2 x B22
  impr_strassen_aux(G, G1X, GX2, //P2
               C, C1X, CX2, //S2
               B, B2X, BX2,
               G,G2X,GX2,
               n2);
...
// C12 = P1 + P2
  ...
  // S5 = A11 + A22
  sum_matrix_blocks(G,G1X,GX1,
                    A,A1X,AX1,
                    A,A2X,AX2,
                    n2);

  // S6 = B11 + B22
  sum_matrix_blocks(G,G1X,GX2,
                    B,B1X,BX1,
                    B,B2X,BX2,
                    n2);
