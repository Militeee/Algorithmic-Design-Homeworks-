...
float ***G=(float ***)malloc(sizeof(float **)*10);
  for (int i=0; i<3; i++) {
    G[i] = allocate_matrix(n2, n2);
  }
...
// C12 = P1 + P2
  ...
  // S5 = A11 + A22
  sum_matrix_blocks(G[0],0,0,
                    A,A1X,AX1,
                    A,A2X,AX2,
                    n2);

  // S6 = B11 + B22
  sum_matrix_blocks(G[1],0,0,
                    B,B1X,BX1,
                    B,B2X,BX2,
                    n2);
