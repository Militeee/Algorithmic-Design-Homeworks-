...
public:
  ...
  // return the edges of a node (actually only the final number of the pair
  // (source,dest))
  ...
  void setColor(int node, std::string col)
  {
    color[node] = col;
    return;
  }
  void setPred(int node, int pred)
  {
    predecessors[node] = pred;
    return;
  }
  void setDist(int node, int dist)
  {
    distance[node] = dist;
    return;
  }
