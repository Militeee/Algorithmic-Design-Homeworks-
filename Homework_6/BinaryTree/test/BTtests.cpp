...
{
  // initialize the trees which is also a test of the custom constructor and
  // insert method
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    bt2.insert(
      std::pair<const std::string, double>{ values[keys[i]], keys[i] + 0.1 });
  }
  ...
  SECTION("Test [] operators (const and not)")
  {
    ...
    REQUIRE(*bt.find(12) ==
            (std::pair<const int, std::string>{ 12, std::string{} }));
    REQUIRE(*bt2.find("baobab") ==
            (std::pair<const std::string, double>{ "baobab", 0.0 }));
  ...
  SECTION("Test the iterators")
  {
    ...
    REQUIRE(bt2.end() ==
            (BinaryTree<std::string, double>::Iterator{ nullptr }));
    ...
    REQUIRE(bt.cend() ==
            (BinaryTree<int, std::string>::ConstIterator{ nullptr }));
    REQUIRE(bt2.cend() ==
            (BinaryTree<std::string, double>::ConstIterator{ nullptr }));
